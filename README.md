...
В рамках рефакторинга `UniversalInput`:

- упрощена логика обновления значений и устранены дублирующиеся вызовы `onChange`;
- исправлена синхронизация `value` между внешними props и внутренним состоянием;
- добавлено сохранение значений в `localStorage`;
- реализована синхронизация значений между вкладками браузера через событие `storage`;
- добавлена обработка ошибок чтения и записи `localStorage`;
- исправлена работа `CodeEditor`, `ref`, `rows` и вставки Tab;
- удалено использование deprecated `document.execCommand`;
- оптимизирован пересчёт ширины блока actions через `ResizeObserver`;
- убрана зависимость от `lodash`;
- исправлены edge cases для `number`, `mask`, `select`, `readOnly` и `autoFocus`;
- добавлена валидация props через `PropTypes`;
- устранены ошибки ESLint.
